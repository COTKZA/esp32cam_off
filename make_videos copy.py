import os
import cv2
import mysql.connector
from datetime import datetime
import time
import requests

# กำหนดเส้นทางโฟลเดอร์
uploads_dir = 'uploads/'
videos_dir = 'videos/'

# ตั้งค่าการเชื่อมต่อ MySQL
db = mysql.connector.connect(
    host='localhost',       # แทนที่ด้วย host ของคุณ
    user='root',           # แทนที่ด้วยชื่อผู้ใช้ของคุณ
    password='',           # แทนที่ด้วยรหัสผ่านของคุณ
    database='esp32cam_video'  # แทนที่ด้วยชื่อฐานข้อมูลของคุณ
)

cursor = db.cursor()

def create_video(image_files):
    # กำหนดชื่อไฟล์วิดีโอ
    video_name = f"video_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
    video_path = os.path.join(videos_dir, video_name)

    # กำหนด codec และสร้าง VideoWriter
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    first_image = cv2.imread(image_files[0])
    height, width, _ = first_image.shape
    video_writer = cv2.VideoWriter(video_path, fourcc, 24, (width, height))

    # เพิ่มภาพลงในวิดีโอ
    for image_file in image_files:
        img = cv2.imread(image_file)
        video_writer.write(img)

    video_writer.release()
    return video_path

def save_video_to_db(video_name, video_path):
    now = datetime.now()
    date = now.date()
    time = now.time()
    cursor.execute("""
        INSERT INTO videos (name, path_file, date, time) 
        VALUES (%s, %s, %s, %s)
    """, (video_name, video_path, date, time))
    db.commit()

def get_unprocessed_images():
    # ดึงภาพที่ยังไม่ถูกใช้งานในการสร้างวิดีโอ
    cursor.execute("SELECT name, path_file FROM images WHERE use_images = 0 GROUP BY name LIMIT 60")
    return cursor.fetchall()

def mark_images_as_used(image_names):
    # อัพเดตสถานะ use_images ของภาพที่ถูกใช้งาน
    cursor.execute("UPDATE images SET use_images = 1 WHERE name IN (%s)" % ', '.join(['%s'] * len(image_names)), image_names)
    db.commit()

def get_image_count_from_api():
    # ดึงข้อมูลภาพจาก API
    response = requests.get("http://192.168.100.165/esp32cam_off/api/api_video.php")
    if response.status_code == 200:
        data = response.json()
        count = sum(1 for image in data if image['use_images'] == 0)
        return count
    else:
        print(f"Error fetching data from API: {response.status_code}")
        return 0

def main():
    while True:  # ลูปเพื่อทำงานต่อไปเรื่อย ๆ
        # ดึงจำนวนภาพที่ยังไม่ถูกใช้งานจาก API
        unprocessed_count = get_image_count_from_api()
        remaining_images = 60 - unprocessed_count

        # ตรวจสอบว่ามีรูปภาพเพียงพอหรือไม่
        if unprocessed_count < 60:
            print(f"ไม่พบรูปภาพเพียงพอในการสร้างวิดีโอ (ต้องมี 60 รูป) เหลือ {remaining_images} รูป")
            time.sleep(10)  # รอ 10 วินาทีก่อนตรวจสอบใหม่
            continue

        # สร้างวิดีโอจาก 60 รูป
        image_paths = [image[1] for image in get_unprocessed_images()]  # ใช้ 60 รูปแรก
        video_path = create_video(image_paths)
        video_name = os.path.basename(video_path)

        # บันทึกข้อมูลวิดีโอในฐานข้อมูล
        save_video_to_db(video_name, video_path)

        # อัพเดตสถานะ fill ของภาพที่ถูกใช้งาน
        image_names = [image[0] for image in get_unprocessed_images()]  # ดึงชื่อภาพ
        mark_images_as_used(image_names)

        print(f"สร้างวิดีโอ: {video_name} และบันทึกลงในฐานข้อมูล")

        time.sleep(10)  # รอ 10 วินาทีก่อนเริ่มรอบถัดไป

if __name__ == "__main__":
    main()

# ปิดการเชื่อมต่อกับฐานข้อมูล
cursor.close()
db.close()
